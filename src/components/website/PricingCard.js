...
function PricingCard({
  ...
  return (
    <div className="flex flex-col border py-2 px-4 rounded-lg w-full sm:w-[409px] h-auto sm:h-[600px] bg-white">
      {/* Header */}
      {!isPopular ? (
        <span className="font-semibold mt-2 mb-2">{planName}</span>
      ) : (
        <div className="flex justify-between items-center mb-2">
          <span className="font-semibold mt-2">{planName}</span>
      ...
      <div className="flex mb-4">
        ...
        <span className=" font-light self-end">{t("pricing.perMonth")}</span>
      </div>
      ...
      <div className="flex flex-col gap-3 flex-1 overflow-y-auto">
        <h4 className="font-bold ml-1">{t("pricing.featuresTitle")}</h4>
